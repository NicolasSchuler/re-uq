...
| --- | --- | --- | --- |
| **PURE** ([Zenodo 7118517](https://zenodo.org/records/7118517)) | 79 public requirements documents, 34,268 sentences; 19 documents in a common XML format with structure (sections, headings) | Real, in-document requirements with the surrounding context (headings, document structure) the context-envelope track needs; would also give natural seeds for a non-template benchmark | No explicit license; curated by ISTI-CNR, contact curator before redistribution |
...
| --- | --- | --- |
| Context envelope (headings, status, neighbours around the item) | PURE (real documents with structure), then the Ferrari et al. package (documented user stories in context) | Envelope builder + fractional design (code, not data) |
| Natural weak-intent statements | Pan & Maalej (wish-shaped reviews), Dollmann & Geierhos (modality-annotated), StorySeek | Commitment-strength annotation to the paper's four-level scale, by two raters, reusing the construct-review protocol |
