...
- RQ4 diagnostic: Can a model detect, in a blind source-grounded text-audit prompt, that its own extracted requirement strengthened or weakened the source modality?
...
- A blind source-grounded text-audit diagnostic, plus declared-modality anchoring ablations, that tests whether models recognize modality strengthening after extraction.
...
- Use denominator-specific risk names in the paper: `unsupported mandatory acceptance@tau` for Task 1, and `HC-OC_all@tau`, `HC-OC_overcommittable@tau`, `weak strengthening@tau`, and `label-correct text overcommitment@tau` for Task 2.
- Make generated-text modality drift a headline Task 2 result, but report strict parser evidence separately from the generic system-verb fallback.
...
- Whether blind Task 3 detects text-level over-commitment, and how much declared-modality anchoring increases false-preserve judgments.
