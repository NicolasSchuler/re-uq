...
| Benchmark construction | `benchmark_manifest*.json` (SHA-256 of every seed file, prompt and benchmark CSV), `benchmark_statements_review*`, `included_capabilities_review*`, `modality_template_inventory.*`, `weak_modality_template_sanity_check.*` | notebooks 01 and 02b via `scripts/populate_notebooks.py`; `eval_utils.write_main_modality_template_inventory` |
